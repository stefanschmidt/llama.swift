...
- (void)runWithPrompt:(nonnull NSString*)prompt config:(nonnull _LlamaRunnerBridgeConfig *)config
{
  ...
  gpt_params params;
  const char *modelPathCString = [_modelPath cStringUsingEncoding:NSUTF8StringEncoding];
  params.model = modelPathCString;

  const char *promptCString = [prompt cStringUsingEncoding:NSUTF8StringEncoding];
  params.prompt = promptCString;

  gpt_params_parse(config, params);
